...
class ControllerCommonHeader extends Controller {
	...
	protected function addCategories(&$data, $categories, $path = '') {
		foreach($categories as $category) {
			...
			if(isset($this->request->get['path'])){
				if((string)$this->request->get['path'] === (string)$_path){
					$active = true;
				}else{
					$active = false;
				}
			...
			if ($this->config->get('config_product_count') && (int)$category['parent_id'] > 0) {
				$total = array(
					'filter_category_id'  => $category['category_id'],
					'filter_sub_category' => true
				);

				$product_total = $this->model_catalog_product->getTotalProducts($total);
